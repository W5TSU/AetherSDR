...
try {
    ...
    Assert-True ((& $planScript @inputs).releaseArtifacts) 'Fork attaches build artifacts to its own v* tag release'
    Assert-True (-not (& $planScript @inputs).productionDraft) 'Fork must not stage a Store production draft'
    $inputs.Ref = 'refs/heads/main'
    Assert-True (-not (& $planScript @inputs).releaseArtifacts) 'A non-tag push never attaches release artifacts'
