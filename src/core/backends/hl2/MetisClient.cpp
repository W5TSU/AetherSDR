    ...
    for (const std::size_t fs : frameStarts) {
        ...
        if (const auto resp = parseEp6Response(bytes.data() + fs)) {
            if (resp->ack) {
                ...
                //
                // Two independent ack consumers, both echo-matched against
                // their own outstanding request and both no-ops when this
                // ack is not theirs: the I/O Board's I2C jobs (ticket #11,
                // its own dispatch queue) and the general RQST/ACK machine
                // (Hl2ControlRequest, docs/HERMES.md §13 item 13). Neither
                // mechanism can mistake the other's ack for its own —
                // decodeI2cResponse() only matches an I2C bus 1/2 reply, and
                // Hl2ControlRequest only matches its one outstanding RADDR.
                if (m_i2cJobs.awaiting()) {
